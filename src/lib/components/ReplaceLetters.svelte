<script lang="ts">
   ...
   // Translate a single text node from its stored original
   function translateTextNode(textNode: Node): void {
      const originalText = originalTextMap.get(textNode) ?? "";

      let translatedText = "";
      for (const char of originalText) {
         const translated = lookupTable[effectiveLocale]?.[char.toLowerCase()] ?? char;
   ...
   function initialScanAndTranslate(): void {
      ...
      console.log("Initial scan and translate completed in", `${end - start}ms`);
   }
   ...
   function retranslateAll(): void {
      ...
      const end = performance.now();
      console.log("Re-translated all text in", `${end - start}ms`);
   }
   ...
   function setupMutationObserver(): void {
      ...
      mutationObserver = new MutationObserver((mutations) => {
         ...
         const end = performance.now();
         console.log("Processed mutations in", `${end - start}ms`);
      });
