<script lang="ts">
   ...
   // Translate a single text node from its stored original (entire text node as one unit)
   function translateTextNode(textNode: Node): void {
      ...
      // Translate the entire text node content as one unit
      // Use trimmed version for lookup but preserve original whitespace
      const trimmedText = originalText.trim();
      const translated = lookupTable[effectiveLocale]?.[trimmedText.toLowerCase()] ?? originalText;
   ...
   function initialScanAndTranslate(): void {
      ...
      console.log("Initial text node scan and translate completed in", `${end - start}ms`);
   }
   ...
   function retranslateAll(): void {
      ...
      const end = performance.now();
      console.log("Re-translated all text nodes in", `${end - start}ms`);
   }
   ...
   function setupMutationObserver(): void {
      ...
      mutationObserver = new MutationObserver((mutations) => {
         ...
         const end = performance.now();
         console.log("Processed text node mutations in", `${end - start}ms`);
      });
      ...
      console.log("Text node MutationObserver started");
   }
   ...
   function cleanupMutationObserver(): void {
      if (mutationObserver) {
         ...
         console.log("Text node MutationObserver stopped");
      }
   }
